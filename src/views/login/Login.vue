<template>
  <h1>网易云音乐</h1>
  <a-form :label-col='labelCol' :rules='rules' :wrapper-col='wrapperCol'>
    <a-form-item v-bind='validateInfos.name' label='用户名 ' name='username'>
      <a-input v-model:value='modelRef.name' />
    </a-form-item>
    <a-form-item v-bind='validateInfos.psw' label='密码' name='password'>
    ...
    const onSubmit = () => {
      validate().then(async res => {
        ...
        if (loginRes.code === 200) {
          ...
          userInfo.id = loginRes.account.id
          // 保存到Storage中
